...
export const ListedDataSchema = z.object({
  ...
  subtitle: z.string().min(3, "Must have a subtitle with at least 3 characters")
    .optional(),
  description: z.string().min(
    3,
    "Must have a description with at least 3 characters",
  ),
});
...
export const UserDetailsFormSchema = z.object({
  yearsOfExperience: z.string().regex(/^\d+$/, "Must be a number").transform(
    Number,
  ),
  experience: z.array(ListedDataSchema),
  skills: z.array(
    z.string().transform((v) => v.trim()).pipe(
      z.string()
        .regex(
          /^[^,;:"']*$/,
          "Skill must not contain any of the following characters (, ; : \" ')",
        ).min(1, "Skill must have at least one character"),
    ),
...
export type UserProfile = UserForm & UserDetailsForm;
